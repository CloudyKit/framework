...
func (manager *Manager) GCinvoke(c *cdi.DI, now time.Time, goroutine bool) {
	if goroutine {
...
func (manager *Manager) GCinvokeifnecessary(c *cdi.DI, goroutine bool) bool {
	now := time.Now()
	manager.lock.Lock()
...
func (manager *Manager) Open(ctx *cdi.DI, sessionName string, dst interface{}) error {
...
func (manager *Manager) Save(ctx *cdi.DI, sessionName string, src interface{}) error {
...
func (manager *Manager) Unregister(ctx *cdi.DI, sessionName string) error {
